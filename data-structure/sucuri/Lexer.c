...
#define SYNTAX_ERROR(pos, erroMsg)           \
	fprintf(stderr, "    %s\n", linha);      \
	fprintf(stderr, "    %*c^\n", pos, ' '); \
	ERRO(criaFilaObjetos em Lexer.c          \
		 : erroMsg);
...
/* Verifica se o string é um delimitador */
bool isDelimiter(char ch)
{
	return (strchr(whiteSpace, ch) || strchr(carOperadores, ch) || ch == '(' || ch == ')' || (ch != '.' && ispunct(ch)));
}

/* Verifica se o string é um operador (ou parênteses) */
bool isOperator(char ch)
{
	return (strchr(carOperadores, ch) || ch == '(' || ch == ')');
}

/* Verifica se o string é um identificador válido */
bool validIdentifier(char *str)
{
	return (isalpha(str[0]));
}

/* Verifica se o string é um número inteiro */
...
/* Verifica se o string é um número real */
bool isRealNumber(char *str)
{
	...
	for (i = 0; i < len; i++)
	{
		if ((!isdigit(str[i]) && str[i] != '.') || (str[i] == '-' && i > 0))
			return false;
		if (str[i] == '.' && hasDecimal)
		{ // Garante que o string só contenha um '.'
			return false;
		}
		if (str[i] == '.')
			hasDecimal = true;
	...
	while (right <= len && left <= right)
	{
		...
		if (isDelimiter(linha[right]) && left == right)
		{
			...
			if (isOperator(linha[right]))
			{
				...
				if (j == MAX_SIMBOLOS)
				{ /* Não encontrou o operador... erro */
					SYNTAX_ERROR(right, ErroSintaxe
								 : sintaxe inválida !);
				}
				else
				{
		...
		{ // Está tudo certo, então adicionamos o objeto na fila
